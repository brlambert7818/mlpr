...
Ws = []
Bs = []
Pred_train = np.zeros((y_train.shape[0], K))
Pred_val = np.zeros((y_val.shape[0], K))
...
for kk in range(K):
    ...
    labels_val = y_val > thresholds[kk] # labels for val set
    ...
    aa_train = np.dot(w_fit_temp[np.newaxis, :], X_train.T).T + b_fitG_temp
    ...
    aa_val = np.dot(w_fit_temp[np.newaxis, :], X_val.T).T + b_fitG_temp
